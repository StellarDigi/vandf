...
if( ! class_exists( 'avia_sc_tab', false ) )
{
	class avia_sc_tab extends aviaShortcodeTemplate
	{
		...
		public function av_tab( $atts, $content = '', $shortcodename = '' )
		{
			 ...
			 * ===========
			 *
			 * currently we leave tabindex=0 to allow to tab through tabs and open with enter
			 * as we do not support the default key behaviour in js because we loose focus
			 * (needs a different implementation)
			 */
			$aria_content = 'aria-hidden="true"';
			$aria_tab = 'aria-selected="false" tabindex="0"';

			if( is_numeric( avia_sc_tab::$initial ) && avia_sc_tab::$counter == avia_sc_tab::$initial )
			{
				$aria_content = 'aria-hidden="false"';
				$aria_tab = 'aria-selected="true" tabindex="0"';
			}
			...
			$output .=		"<div id='{$atts['custom_id']}-tab' class='{$tab_class}' role='tab' {$aria_tab} data-fake-id='#{$atts['custom_id']}' aria-controls='{$atts['custom_id']}-content' {$markup_title}>{$icon}{$atts['title']}</div>";
			$output .=		"<div id='{$atts['custom_id']}-content' class='{$tab_content_class}' role='tabpanel' aria-labelledby='{$atts['custom_id']}-tab' {$aria_content}>";
